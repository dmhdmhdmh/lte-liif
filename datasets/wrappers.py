...
from utils import to_freq_samples
...
class SRImplicitDownsampled(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #hr_shape = np.array([crop_hr.shape[0],crop_hr.shape[1],crop_hr.shape[2]])
        #freq_lr = fft.fftn(crop_lr, dim=(1,2))
        ...
        hr_coord, hr_rgb = to_pixel_samples(crop_hr.contiguous())
        #freq_hr_coord, freq_hr_spec = to_freq_samples(freq_hr.contiguous())

        if self.sample_q is not None:

            sample_lst_hr = np.random.choice(
                len(hr_coord), self.sample_q, replace=False)

            '''
            sample_lst_freq_hr = np.random.choice(
                len(freq_hr_coord), len(freq_hr_coord), replace=False)
            '''
            #sample_lst_hr = np.arange(0, len(hr_coord), 1)
            #sample_lst_freq_hr = np.arange(0, len(freq_hr_coord), 1)
            hr_coord = hr_coord[sample_lst_hr]
            hr_rgb = hr_rgb[sample_lst_hr]
            #freq_hr_coord = freq_hr_coord[sample_lst_freq_hr]
            #freq_hr_spec = freq_hr_spec[sample_lst_freq_hr]
